...
def execute_spider():
    if dataset_entry.get() == '' or chosen_feed not in ['DB', 'CSV', 'JSON']:
    ...
    if chosen_feed == 'DB':
        print ("PATHHHH",folder_path)
        try:
            feed_uri = f"sqlite:///{folder_path}/{dataset_entry.get()}.{chosen_feed.lower()}"
        except:
            messagebox.showerror('Error', 'All entries are required')
    else:
        try:
    ...
    configure_logging()
    runnner = CrawlerRunner(settings)
    runnner.crawl(chosen_spider)
