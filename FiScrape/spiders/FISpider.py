...
class FT_Spider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        if author_urls:
            yield from response.follow_all(author_urls, callback=self.parse_author, meta={'article_item': article_item})
        else:
    ...
    def parse_author(self, response):
        ...
        yield author
        self.add_authors(response, author)
        
    def add_authors(self, response, author):
        article_item = response.meta['article_item']
        loader = ItemLoader(item=article_item, response=response)
